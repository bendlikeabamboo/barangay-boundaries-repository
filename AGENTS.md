...
PSGC (Philippine Standard Geographic Code) snapshot processor. Converts government Excel/PDF snapshots into RDF linked data (W3C ORG ontology) and reconciles NAMRIA shapefile boundaries with PSGC codes to produce enriched GeoJSON.
...
namria/phl_admbnda_adm{0-4}_*.shp (version 2023-11-06)
  ...
  → reconcile.py                 — fuzzy name matching (rapidfuzz token_set_ratio/token_sort_ratio) on ADM3/ADM4
...
| `brgybnd reconcile --diff diff.json` | cli.py:444 | Fuzzy name reconciliation |
...
Highly Urbanized Cities have PSGC parent = region (not province), but NAMRIA places them under provinces. This mismatch is handled by `huc_adm2_mapping.json` (generated by `generate_huc_mapping.py`), consumed by `coverage.py`, `reconcile.py`, and `enrich.py`. Metro Manila also lacks PSGC province-level units — NAMRIA's 4 NCR legislative districts are mapped as `virtual_provinces` of type `mm_district`.

## Name Sanitization

Duplicated in 3 files (reconcile.py:54-68, enrich.py:35-42, generate_huc_mapping.py:180-193). Removes "(Capital)", "(Pob.)", "City of" prefix, normalizes diacritics/punctuation.
...
barangay_boundaries_repository/
  ...
  reconcile.py              — fuzzy name matching (rapidfuzz)
